...
const EDUCATION_LEVELS = ["Elementary","Junior High School","Senior High School","Vocational / Technical","College","Post-Graduate"];
const UNITS_EARNED_OPTIONS = ["Undergraduate","Graduate","Post-Graduate","Completed"];
const MONTHS = ["January","February","March","April","May","June","July","August","September","October","November","December"];
const BLANK_EDUCATION = { education_level:"", school_name:"", basic_educ_degree_course:"", attendance_start_date:"", attendance_end_date:"", highest_level_units_earned:"", year_graduated:"", scholarship_academic_honors:"" };
...
function FloatingSelect({ id, label, value, onValueChange, options, className = "", error }: {
...
function FloatingDatePicker({ id, label, value, onChange, className = "", error }: {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {/* ✅ Label on border line — identical to FloatingInput */}
      <label htmlFor={id}
...
export default function EducationSection({ records, isOpen, onToggle, onArrayChange, onAdd, onDelete }: EducationSectionProps) {
  return (
    <Accordion type="single" collapsible value={isOpen ? "education" : ""} onValueChange={() => onToggle()} className="w-full">
      <AccordionItem value="education" className="border border-border rounded-[14px] overflow-hidden bg-card">
        ...
        <AccordionContent className="px-4 pb-4 pt-2 bg-background">
          {records.map((edu: any, i: number) => {
            ...
            return (
              <div key={edu.education_id ?? i} className="relative mb-4 p-4 rounded-[12px] border border-border bg-card">
                <button className="absolute top-3 right-3 p-1.5 rounded-[8px] hover:bg-destructive/10 hover:text-destructive transition-colors duration-200" onClick={() => onDelete("emp_education", i)}><Trash2 size={14} /></button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <FloatingSelect id={`education_level_${i}`} label="Education Level" value={levelValue} onValueChange={(v) => onArrayChange("emp_education", i, "education_level", v)} options={EDUCATION_LEVELS} className="sm:col-span-2" />
                  <FloatingInput id={`school_name_${i}`} label="School Name" value={edu.school_name || ""} onChange={(v) => onArrayChange("emp_education", i, "school_name", v)} className="sm:col-span-2" />
                  <FloatingInput id={`degree_${i}`} label="Degree / Course" value={edu.basic_educ_degree_course || ""} onChange={(v) => onArrayChange("emp_education", i, "basic_educ_degree_course", v)} className="sm:col-span-2" />
                  <SectionDivider label="Attendance Period" />
                  <FloatingDatePicker id={`start_date_${i}`} label="Start Date" value={edu.attendance_start_date || ""} onChange={(v) => onArrayChange("emp_education", i, "attendance_start_date", v)} />
                  <FloatingDatePicker id={`end_date_${i}`} label="End Date" value={edu.attendance_end_date || ""} onChange={(v) => onArrayChange("emp_education", i, "attendance_end_date", v)} />
                  <SectionDivider label="Academic Info" />
                  <FloatingSelect id={`units_${i}`} label="Units Earned" value={edu.highest_level_units_earned || ""} onValueChange={(v) => onArrayChange("emp_education", i, "highest_level_units_earned", v)} options={UNITS_EARNED_OPTIONS} />
                  <FloatingInput id={`year_grad_${i}`} label="Year Graduated" value={edu.year_graduated || ""} onChange={(v) => onArrayChange("emp_education", i, "year_graduated", v)} />
                  <FloatingInput id={`honors_${i}`} label="Scholarship / Honors" value={edu.scholarship_academic_honors || ""} onChange={(v) => onArrayChange("emp_education", i, "scholarship_academic_honors", v)} className="sm:col-span-2" />
                </div>
              </div>
            );
          })}
          <button className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-[10px] border-[1.5px] border-dashed border-input hover:border-primary/50 hover:bg-primary/5 transition-all duration-200 text-sm font-medium text-muted-foreground hover:text-primary" onClick={() => onAdd("emp_education", { ...BLANK_EDUCATION, education_id: `new-${Date.now()}` })}>
